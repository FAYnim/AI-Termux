...
export class ConfigManager {
   ...
   *
   * BUG-03: in-memory cache invalidated on `saveConfig()`. Caveat: external
   * file writes (e.g. tests that call `fs.writeFileSync(config.json)`)
   * bypass the cache. Tests using tempdir-backed ConfigManager instances
   * already call `saveConfig()` (or instantiate a fresh manager) so this
   * is not an issue in current tests. If a test mutates the file without
   * going through saveConfig, instantiate a new ConfigManager.
   *
   * @returns {object}
   */
  loadConfig() {
    ...
    // BUG-03: cache hit when no writes have happened
  ...
  saveConfig(configData) {
    ...
    // BUG-03: refresh cache eagerly so the next loadConfig() call returns
