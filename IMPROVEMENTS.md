...
- **Status**: ✅ **FINISHED** (2026-08-31) — New `src/llm/http-pool.js` exports `pooledFetch`, a `fetch`-compatible wrapper that lazily constructs a single shared undici `Agent` (256 connections, 60s keep-alive, pipelining 1) via `import('undici')` when the package is reachable. `BaseLlmClient` and `GeminiClient` now default `this.fetch` to `pooledFetch` instead of `globalThis.fetch`; injected fetches (used by tests and `_handleErrorResponse` mocks) still win. When `undici` is not installed, the wrapper falls back to `globalThis.fetch` — Node's global fetch already pools internally via its built-in undici, so ReAct loops still benefit from connection reuse without adding a runtime dep. Zero new dependencies: the lazy import means the project stays at zero deps and gains real, configurable pooling the moment `undici` is added. 7 new tests in `tests/http-pool.test.js` and `tests/llm-pooled-fetch.test.js` lock fallback behavior, dispatcher cache identity, idempotent `closePool`, init-passthrough, and that both LLM clients wire the wrapper by default. Suite 547/556 (same 9 pre-existing wizard failures).
